...
function finishedReplication() {
    ...
    if (!validateRadioButtons("how_effective_is_the_new_medication")) {
        ...
        return false;
    } else if (!validateRadioButtons("does_the_medication_really_reduce_illness")){
...
function finishedComprehension() {
    ...
    if (x === '') {
        ...
        if (radios[1].checked) {
            document.getElementById("explain_no_answer").style.display = 'inline';            
        }
...
function finishedPreviousStudies() {
    ...
    if (document.getElementsByName("does_the_medication_really_reduce_illness")[1].checked && !document.forms["answers"]["why_no"].value) {
        alert("Your answer for the first question is missing.");
        return false;
    } else if (!validateRadioButtons("participated_in_previous_study")) {
