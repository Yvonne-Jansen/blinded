...
if (isset($_POST['start_code'])) {
	...
	if (strpos($data, "," . $start_code . ",") !== false) {
		...
		$invalid_code = true;
	} else if (hexdec($start_code) < 700000 or hexdec($start_code) > 705000){
		$code = 'invalid code (study was started with invalid url)';
		$invalid_code = true;
	} else if ($time_taken < 30){
...
if (isset($_POST['does_the_medication_really_reduce_illness'])) {
$really_reduce = $_POST['does_the_medication_really_reduce_illness'];
$out .= $really_reduce . ",";
}
...
if ($correct_trial and !$duplicate_correct and !$reloader_cheat and !$invalid_code and !$did_study_before) {
